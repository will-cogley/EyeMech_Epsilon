...
blink_time = 50
blinking = False
static_timer_last = static_timer
...
while True:
    ...
    if staticflag == False: # If face was detected
        static_timer = static_target # reset idle mode countdown
        bypassflag = False
    else: 
        if static_timer != 0:
            static_timer -= 1
            bypassflag = False
        else:                      # Enter Idle mode if no face detected for a while
            if new_idle:
                bypassflag = True
                servos["LR"].write(random.randint(80, 100))
                control_ud_and_lids(random.randint(servo_limits["UD"][0]+40,servo_limits["UD"][1])-40)
                idle_pause = random.randint(50,300)
                new_idle = False
            else:
                idle_pause -= 1
                if idle_pause <= 0:
    ...
    if not blinking and random.randrange(500) == 0: # Check whether to randomly trigger a blink
        blinking = True
...
#     print(x_offset)
    ...
    if blinking:
        ...
        if (x_offset < -deadzone or x_offset > deadzone) and not bypassflag:
#             print("hello")
        ...
        if (y_offset < -deadzone or y_offset > deadzone)and not bypassflag:
